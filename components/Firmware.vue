    ...
    <Teleport to="body">
      ...
      >
        ...
        <template v-else>
          ...
          <div
            v-if="showPreviewFirmware && store.$state.previews.length > 0"
            class="px-4 py-2 text-sm text-meshtastic font-semibold border-theme-bottom"
          >
...
const showPreviewFirmware = computed(() => {
  return store.prereleaseUnlocked || store.couldntFetchFirmwareApi || store.$state.usesStaticForkReleases
})
